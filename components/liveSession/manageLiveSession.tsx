...
import { MentorSession } from "../../service/api/liveSession.api";
...
const manageLiveSession = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = sessionList.filter(session => {
        ...
        const mentorName = mentor ? mentor.name.toLowerCase() : '';
    ...
    const mapped = filtered.map(session => {
      ...
      const imageUrl = mentor ? mentor.image : "/images/dummy-mentor.png";
  ...
  }, [sessionList, mentors, searchQuery]);
  ...
  const handleAddSession = async (values: any) => {
    try {
      ...
      const sessionData = {
        mentor_id: values.mentor, // This is the mentor id from the dropdown
  ...
  const initialValues = selectedSession ? {
    ...
    mentor: selectedSession.mentor_id,
    dateTime: dayjs(`${selectedSession.date} ${selectedSession.time}`, 'YYYY-MM-DD HH:mm'),
